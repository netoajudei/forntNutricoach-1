...
const SummaryCard: React.FC<{ title: string; children: React.ReactNode; linkTo?: string; className?: string }> = ({ title, children, linkTo, className }) => {
    const content = (
         <Card className={`hover:border-brand-500 hover:shadow-lg transition-all h-full ${className}`}>
            <h3 className="font-semibold text-gray-500 dark:text-gray-400 text-sm mb-2">{title}</h3>
            {children}
        </Card>
    );
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8">
            <div className="max-w-7xl mx-auto">
                <h1 className="text-3xl font-bold tracking-tight mb-6">Seu Progresso</h1>
...
export const ProgressoPesoPage: React.FC = () => {
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8">
            <div className="max-w-7xl mx-auto">
                <Link to="/progresso" className="text-sm text-brand-600 hover:underline mb-2 inline-block">&larr; Voltar para Progresso</Link>
                    ...
                    {isLoading || !history ? <Skeleton className="h-full w-full" /> : (
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={history} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                                ...
                                <Tooltip />
                                <Legend />
                                <Line type="monotone" dataKey="weight" name="Peso (kg)" stroke="#3b82f6" strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 8 }}/>
                            </LineChart>
                        </ResponsiveContainer>
                    )}
                    </div>
                        ...
                        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                {isLoading || !history ? (
                                    ...
                                    history.map((entry) => (
                                        <tr key={entry.date} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                            <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{entry.date}</td>
        ...
        return (
            <ResponsiveContainer width="100%" height={320}>
                <BarChart data={data}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Bar dataKey="value" name={name} fill="#3b82f6" />
                </BarChart>
            </ResponsiveContainer>
        );
    };
